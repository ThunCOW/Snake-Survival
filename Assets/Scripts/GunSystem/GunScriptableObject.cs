...
#pragma warning restore
            ...
            case ProjectileType.LaserBullet:
                ...
                projectile.OnCollision += HandleLaserGun;
                projectile.StartPoint = StartPoint;
                projectile.EndPoint = EndPoint;
                instance.transform.position = StartPoint;
                instance.transform.GetChild(0).gameObject.SetActive(true);
...
#pragma warning restore
    ...
    private void HandleLaserGun(Projectile projectile, Collider collider)
    {
        LayerMask mask = LayerMask.GetMask("Enemy");
        Collider[] hitColliders = Physics.OverlapSphere(projectile.transform.position, 1, mask);
        foreach (Collider hit in hitColliders)
        {
            if (hit != null)
            {
